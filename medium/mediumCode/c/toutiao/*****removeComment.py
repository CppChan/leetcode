...
#   	return res
  ...
  def removeComments2(self, source):# ordinary, for all /* must have a */
	  ...
	  for line in source:
		  ...
		  if not in_block:
			  newline = []
		  while i < len(line): #remember the sorting of the if statement, cannot first be the if not inblock: newline.append(line[i]), because may be "/*" at the beginning
			  if line[i:i + 2] == '/*' and not in_block:
				  in_block = True
				  i += 1# here add 1, and in the end of the loop, add 1 again, so will pass the "/*"
			  elif line[i:i + 2] == '*/' and in_block:
				  in_block = False
				  i += 1
			  elif not in_block and line[i:i + 2] == '//': # remember not inblock, ow will ignore  //.....*/, then inblock will always be True
				  break
			  elif not in_block:
				  newline.append(line[i])
			  i += 1
		  if newline and not in_block: # remember if newline, ow will append "" into newline
